...
my class X::Snake::CannotBuildModule is Exception {
    has $.cmd;
    has $.module;
    method message {
...
class Build {
    method build(|) {
        ...
        my @core_modules = { :name<Slangy>, :deps([]), :is_nqp },
                           { :name<Snake::World>, :deps([]), :is_nqp },
                           { :name<Snake::Actions>, :deps([<Snake::World>]), :is_nqp },
                           { :name<Snake::Grammar>, :deps([<Snake::Actions Slangy>]), :is_nqp },
                           { :name<Snake::ModuleLoader>, :deps([]), :is_nqp },
                           { :name<Snake>, :deps([<Snake::Actions Snake::ModuleLoader>]), :is_nqp };
        my @build_order;
        ...
        for @core_modules, @build_order -> $module {
            ...
            my $pm         = $is_nqp ?? "src/$basename.pm" !! "src/Snake/$basename.pm";
            my $bc         = ($is_nqp ?? 'lib/' !! 'lib/Snake/') ~ "{$basename}.pm.{$*VM.precomp-ext}";
            ...
            shell($cmd).status == 0
                or die X::Snake::CannotBuildModule.new( :$module, :$cmd);
